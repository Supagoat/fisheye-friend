...
//- Mirrors control points L/R (and T/B by default) to enforce symmetry that Hemi exhibits.
//- Bicubic sampling for high quality; outside the fisheye circle -> transparent (or clamp via --clampEdge).
//
//Usage:
//javac HemiTPSDefish.java
//java HemiTPSDefish <input.jpg> <mapping.json> <output.png> [outW outH] [--lambda=1e-5] [--noMirrorY] [--clampEdge]
//
//Mapping JSON must look like your shared file with fields:
//"source":{"width":Wsrc,"height":Hsrc}, "target":{"width":Wdst,"height":Hdst}, "pairs":[{ "src":{x,y}, "dst":{x,y} }, ... ]
//
//Focused on quality, not speed. No external libraries.
...
public class MapDefishV5 {

 // ---------- CLI ----------
 public static void main(String[] args) throws Exception {
     if (args.length < 3) {
         System.out.println("Usage: java HemiTPSDefish <input.jpg> <mapping.json> <output.png> [outW outH] [--lambda=1e-5] [--noMirrorY] [--clampEdge]");
         return;
     }
     ...
     BufferedImage src = readFileToIntBI(inPath);//ImageIO.read(new File(inPath));
     if (src == null) throw new IOException("Cannot read input image: " + inPath);
     ...
     ImageIO.write(out, outPath.toLowerCase().endsWith(".jpg") ? "png" : "png", new File(outPath));
     System.out.println("Wrote " + outPath + "  (" + outW + "x" + outH + ")");
 }
 
 public static BufferedImage readFileToIntBI(String filePath) throws IOException {
 ...
 static class Mapping {
     ...
     static Mapping read(File f) throws IOException {
         ...
         Pattern p = Pattern.compile("\\{\\s*\"id\"\\s*:\\s*\\d+[^}]*?\"src\"\\s*:\\s*\\{[^}]*?\"x\"\\s*:\\s*([\\-0-9.]+)[^}]*?\"y\"\\s*:\\s*([\\-0-9.]+)[^}]*?\\}[^}]*?\"dst\"\\s*:\\s*\\{[^}]*?\"x\"\\s*:\\s*([\\-0-9.]+)[^}]*?\"y\"\\s*:\\s*([\\-0-9.]+)", Pattern.DOTALL);
 ...
 static class ThinPlateSpline {
     ...
     // Simple symmetric solver with partial pivoting (works for our dense system)
