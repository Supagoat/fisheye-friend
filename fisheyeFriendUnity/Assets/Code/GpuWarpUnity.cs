...
namespace FisheyeFriend {
    ...
    public class GpuWarpUnity : MonoBehaviour {
        ...
        void Start () {
            ...
            // move to save button  File.WriteAllBytes(config.outPath, readback.EncodeToPNG());

        }
        void OnDestroy () { warper?.Dispose(); }


        public Texture2D WarpImage (Bitmap src) {
            ...
            Texture2D sourceTexture = new Texture2D(src.Width, src.Height);
            ...
            RenderTexture outputRT = new RenderTexture(sourceTexture.width, sourceTexture.height, 0, RenderTextureFormat.ARGB32);
        ...
        public class GpuWarper : System.IDisposable {
            ...
            public GpuWarper (ComputeShader cs) {
                _cs = cs;
                _kernel = _cs.FindKernel("WarpLanc");
            }
